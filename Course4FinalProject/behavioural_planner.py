...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        # check_for_stop_signs() helper functionwaypointss.
        ...
        if self._state == FOLLOW_LANE:
            ...
            # ------------------------------------------------------------------
            # print('1. waypoints[64]:', waypoints[64])
            ...
            # print('2. waypoints[64]:', waypoints[64])
            ...
            # ------------------------------------------------------------------
            # print('3. waypoints[68]:', waypoints[68])
            # print('self._goal_index =', self._goal_index)
            ...
            # DONE: INSERT YOUR CODE BETWEEN THE DASHED LINES
            # ------------------------------------------------------------------
            if stop_sign_found and self._stop_count < STOP_COUNTS:
                self._goal_state[2] = 0.0
                self._state = DECELERATE_TO_STOP
                print('It comes here, stop count is;', self._stop_count)
            # ------------------------------------------------------------------
            # print('Lane follow goal index:', self._goal_index)
            # print('Lane follow waypoint state:', waypoints[goal_index])
            # print('Lane follow goal state:', self._goal_state)
        ...
        # the stop sign and transition to the next state.
            ...
            if self._stop_count == STOP_COUNTS:
                ...
                # DONE: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # NOTE:  here, if there are multiple stop signs along the waypoint? but it seems that it is not considered,
                # --------------------------------------------------------------
            ...
            # Otherwise, continue counting.
                ...
                print('waiting before stop sign, count: ', self._stop_count)
                # --------------------------------------------------------------
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        while wp_index < len(waypoints) - 1:
            dist = math.sqrt((waypoints[closest_index][0] - waypoints[closest_index + 1][0]) ** 2
                             + (waypoints[closest_index][1] - waypoints[closest_index][1]) ** 2)
            arc_length += dist
            if arc_length > self._lookahead:
                break
            wp_index += 1
...
def get_closest_index(waypoints, ego_state):
    ...
    # closest_index = 0
    # DONE: INSERT YOUR CODE BETWEEN THE DASHED LINES
    waypoints_xy = np.array(waypoints)[:, 0:2]
    ego_xy = np.array(ego_state[0:2])
    dist = np.sqrt((waypoints_xy[:, 0] - ego_xy[0]) ** 2 + (waypoints_xy[:, 1] - ego_xy[1]) ** 2)
    closest_len = float(np.amin(dist))
