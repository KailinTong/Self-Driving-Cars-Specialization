...
class PathOptimizer:
    ...
    def optimize_spiral(self, xf, yf, tf):
        ...
        # ------------------------------------------------------------------
        bnds = scipy.optimize.Bounds([-0.5, -0.5, sf_0], [0.5, 0.5, np.inf])
        # ------------------------------------------------------------------
        ...
        # DONE: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        res = scipy.optimize.minimize(fun=self.objective, x0=np.array(p0), bounds=bnds, jac=self.objective_grad,
                                      method='L-BFGS-B')
        # ------------------------------------------------------------------
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        # ...
        thetas = np.dot(np.array(a), np.array(s)) + np.dot(np.array(b) / 2.0, np.array(s) ** 2) + \
                 np.dot(np.array(c) / 3.0, np.array(s) ** 3) + np.dot(np.array(d) / 4.0, np.array(s) ** 4)
        return list(thetas)
        # ------------------------------------------------------------------
    ...
    def sample_spiral(self, p):
        ...
        x = np.cos(t_points)
        x_points = list(scipy.integrate.cumtrapz(x, s_points))
        y = np.sin(t_points)
        y_points = list(scipy.integrate.cumtrapz(y, s_points))
