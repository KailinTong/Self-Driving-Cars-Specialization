    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                x_end = paths[i][0][-1]
                y_end = paths[i][1][-1]
                k_goal = 5 # normalization factor
                score = exp(((x_end - goal_state[0]) ** 2 + (y_end - goal_state[1]) ** 2) / k_goal)
                # print('goal_state:', goal_state)
                # print('x, y, heading of end of Path ', i, ': ', paths[i][0])
                # print('Path ', i, ' score is: ', score)
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # DONE: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            # More away from collision paths, smaller score
                            score += self._weight * 1.0 / fabs(i - j)
                            # --------------------------------------------------
